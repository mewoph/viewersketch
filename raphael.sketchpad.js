...
$(document).ready(function() {
	...
	$("#stroke").change(function() {
		sketchpad.pen().width(this.value);
	});

	$("#opacity").change(function() {
		sketchpad.pen().opacity(this.value);
	});

	$("#pickcolor").change(function() {
		sketchpad.pen().color('#' + this.value);
	});

	$("#export").click(function() {
		return sketchpad.json();
	});
	...
	function update_actions() {
		enable("#clear", sketchpad.strokes().length > 0);
		enable("#undo", sketchpad.undoable());
		enable("#redo", sketchpad.redoable());
	}
